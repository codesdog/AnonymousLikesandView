...
namespace RoutinePlan.GenRoutinePlan
{
    public partial class GenRoutinePlanUserControl : UserControl
    {
        public GenRoutinePlan webObj { get; set; }
                ...
                try
                {
                    ...
                    if (pList != null)
                    {
                        for (int j = 0; j < leftDays.Count; j++)
                        {
                            ...
                            double during = double.Parse(rPlanListItem["计划时长"].ToString());

                            DataTable dtPlans = IsHadActivity(dtStart, during, spWeb, planList, currentUser);
                            DataTable dtAllPlans = dtPlans.Clone();
                            
                            if (dtPlans!=null)
                            {
                                foreach (DataRow dr in dtPlans.Rows)
                                {
                                    dtAllPlans.ImportRow(dr);//复制行数据到新表
                                }
                            }
                            else
                            {
                            ...
                            if (dtAllPlans!=null)
                            {
                                lbWaring.Text = "下列计划时间已有活动安排，无法生成例行计划！";
        ...
        private DataTable IsHadActivity(DateTime dt,double during,SPWeb spWeb, string planList,SPUser currentUser)
        {
            DataTable dtPlans = new DataTable("已有计划表");
            dtPlans.Columns.Add("活动ID");
            dtPlans.Columns.Add("活动对象");
            dtPlans.Columns.Add("计划开始");
            dtPlans.Columns.Add("计划时长");
            dtPlans.Columns.Add("活动Url");
            //dtPlans.Columns.Add("活动对象");
            try
            {
                SPList pList = spWeb.Lists.TryGetList(planList);
                if (pList != null)
                {
                    ...
                    string strStart = SPUtility.CreateISO8601DateTimeFromSystemDateTime(dt);
                    string strEnd = SPUtility.CreateISO8601DateTimeFromSystemDateTime(dt.AddMinutes(during));
                    qry.Query =@"<Where>
                                    <And>
                                      <And>
                                         <Or>
                                            <And>
                                               <Gt>
                                                  <FieldRef Name='PlanEnd' />
                                                  <Value Type='Calculated'>"+strStart+@"</Value>
                                               </Gt>
                                               <Lt>
                                                  <FieldRef Name='PlanEnd' />
                                                  <Value Type='Calculated'>"+strEnd+@"</Value>
                                               </Lt>
                                            </And>
                                            <Gt>
                                               <FieldRef Name='PlanDate' />
                                               <Value Type='DateTime' IncludeTimeValue='TRUE'>"+strStart+@"</Value>
                                            </Gt>
                                         </Or>
                                         <Lt>
                                            <FieldRef Name='PlanDate' />
                                            <Value Type='DateTime' IncludeTimeValue='TRUE'>"+strEnd+ @"</Value>
                                         </Lt>
                                      </And>
                                      <Eq>
                    ...
                    SPListItemCollection listItems = pList.GetItems(qry);
                    DataTable listtable=listItems .GetDataTable();
                    if (listItems.Count>0)//指定开始时间和计划时长与系统已有活动冲突
                    {

                        for (int i = 0; i < listItems.Count; i++)
                        {
                            DataRow dr= dtPlans.NewRow();
                            dr["活动ID"] = listItems[i]["ID"];
                            dr["活动对象"] = listItems[i]["Title"];
                            dr["计划开始"] = listItems[i]["PlanStart"];
                            dr["计划时长"] = listItems[i]["PlanDuring"];
                            dr["活动Url"] = listItems[i].Url;
                            dtPlans.Rows.Add(dr);
                        }
                    }
                    else
